...
function updatePricingQuickView() {    
    ...
    var regex = /([0-9]+[.|,][0-9]+[.|,][0-9]+)/g;		
    var unitPriceTextMatch = jQuery('.quick-view-product .prices .price').data("price");
    if (unitPriceTextMatch) {
        var unitPriceText = unitPriceTextMatch;
        var unitPrice = unitPriceTextMatch;
        var quantity = parseInt(jQuery('.quick-view-product input[name=quantity]').val());
        var totalPrice = unitPriceTextMatch * quantity;				
        var totalPriceText = Haravan.formatMoney(totalPrice,window.money_format);				
        jQuery('.quick-view-product .total-price span').html(totalPriceText);
    }
}
...
var loadIcon = "//hstatic.net/473/1000036473/1000058636/loading.gif?v=1000";
...
//========== QUICK VIEW ==========//
		...
		$(document).on('click', '.quick-view', function(e){
			...
			Haravan.getProduct(producthandle,function(product) {
				...
				if(product.compare_at_price == 0 && product.price == 0){
                    ...
                    quickview.find(".price").html("Liên hệ để xem giá");
	...
	function loadQuickViewSlider(n, r) {
        ...
        if (n.images.length > 0) {
            ...
            for (i in n.images) {
                var u = Haravan.resizeImage(n.images[i], "original");
                var a = Haravan.resizeImage(n.images[i], "original");
				...
				if(dataGroup != "Vendor"){
					...
					$('a', $(this)).each(function(){
						...
						if(!$('body').hasClass('template-collection')){
							$(this).attr('href','/tat-ca-san-pham/'+dataHandle);
							$(this).unbind( "click" );
						}else{
